...
.resume-job-line{margin:8px 0 3px;font-size:11.8px;font-weight:700;color:#111827}
...
const ResumeDocument = forwardRef(function ResumeDocument({
  ...
  return (
    ...
    >
      <header className="resume-header" style={{ textAlign: 'center', marginBottom: '16px', paddingBottom: '12px', borderBottom: '1.5px solid #111827' }}>
        ...
        {headline && (
          <p className="resume-headline" style={{ margin: '7px 0 0', fontSize: '12px', lineHeight: 1.3, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#374151' }}>
            {headline}
          </p>
        )}
        {contacts.length > 0 && (
      ...
      {hasSections ? sections.map((section, sectionIndex) => (
        <section key={`${section.heading}-${sectionIndex}`} className="resume-section" style={{ margin: '0 0 13px', pageBreakInside: 'avoid' }}>
          <h2 className="resume-section-title" style={{ margin: '0 0 4px', paddingBottom: '4px', borderBottom: '1.5px solid #111827', fontSize: '13px', lineHeight: 1.2, fontWeight: 800, letterSpacing: '0.09em', textTransform: 'uppercase', color: '#111827' }}>
          ...
          <div className="resume-section-body" style={{ fontSize: '11.5px', lineHeight: 1.46, color: '#111827' }}>
            {(section.lines || []).map((line, lineIndex) => {
              ...
              if (isBulletLine(text)) {
                return (
                  <div key={lineIndex} className="resume-bullet" style={{ display: 'flex', gap: '8px', margin: '0 0 3px', paddingLeft: '8px', pageBreakInside: 'avoid', fontWeight: 700 }}>
                    ...
                    <span style={{ fontWeight: 700 }}>{stripBullet(text)}</span>
                  </div>
                );
              }

              if (isJobHeaderLine(text)) {
                return <p key={lineIndex} className="resume-job-line" style={{ margin: '8px 0 3px', fontSize: '11.8px', fontWeight: 700, color: '#111827' }}>{text}</p>;
              }
              ...
              return <p key={lineIndex} className="resume-paragraph" style={{ margin: '0 0 4px' }}>{text}</p>;
            })}
          </div>
        </section>
      )) : (
